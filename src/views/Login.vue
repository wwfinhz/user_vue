<template>
  <el-form :model="ruleForm2" :rules="rules2" ref="ruleForm" label-position="left" label-width="0px" class="demo-ruleForm login-container">
    <router-link to="/index"><el-button type="Brand Color" icon="el-icon-arrow-left" round size="mini">返回首页</el-button></router-link>
    <h3 class="title">户主登录</h3>
    ...
    <el-row>
      ...
      <el-col :span="12"><div class="regist"><router-link to="/register" style="color:#F56C6C">注册</router-link></div></el-col>
    </el-row>
...
<script>
  import { loginhuzhu } from '../api/api';
      ...
      handleSubmit2(ev) {
        ...
        this.$refs.ruleForm.validate((valid) => {
          if (valid) {
            ...
            var loginParams = { hzphone: this.ruleForm2.account, 
            hzpassword: this.ruleForm2.checkPass };
            
            // var loginParams = { hzphone: this.ruleForm2.account, hzpassword: this.ruleForm2.checkPass };
            console.log(loginParams);
            loginhuzhu(loginParams).then(res => {
              this.logining = false;
              console.log(res);
              if (res.data.code) {
                this.$message({
              ...
              this.$router.replace({ path: '/fangchan' });
